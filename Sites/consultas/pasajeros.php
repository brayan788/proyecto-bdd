...
<?php
  ...
  require("../config/conexion_91.php");
	...
	$ciudadesa = "SELECT DISTINCT ae.nombre_ciudad FROM aerodromos ae INNER JOIN vuelos vu ON ae.aerodromo_id=vu.aerodromo_salida_id WHERE vu.estado LIKE 'aceptado';";
	$result2 = $db -> prepare($ciudadesa);
	$result2 -> execute();
	$dataCollected = $result2 -> fetchAll();

	$ciudades = "SELECT DISTINCT ae.nombre_ciudad FROM aerodromos ae INNER JOIN vuelos vu ON ae.aerodromo_id=vu.aerodromo_llegada_id WHERE vu.estado LIKE 'aceptado';";
	$result3 = $db -> prepare($ciudades);
	$result3 -> execute();
	$dataCollected1 = $result3 -> fetchAll();

	$fechas = "SELECT DISTINCT fecha_salida FROM vuelos WHERE estado LIKE 'aceptado';";
